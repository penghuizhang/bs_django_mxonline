...
from .forms import LoginForms, RegisterForms, ForgetForms, ResetPwdForms
    ...
    def post(self, request):
        ...
        if register_forms.is_valid():
            ...
            if UserProfile.objects.filter(email=user_name):
                return render(request, "login.html", {"msg": "该账户已经注册","register_forms": register_forms})
...
class ModifyPwdView(View):
    def post(self, request):
        resetForms = ResetPwdForms(request.POST)
        if resetForms.is_valid():
            email = request.POST.get("email", "")
            password = request.POST.get("password", "")
            ...
            return render(request, "password_reset.html", {"msg": resetForms})
